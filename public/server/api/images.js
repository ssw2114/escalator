var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
/* eslint-disable camelcase */
var router = require('express').Router();
var cloudinary = require('cloudinary');
var formData = require('express-form-data');
var cors = require('cors');
var Image = require('../db/models/image');
var Gpx = require('../db/models/gpx');
var EXIFParser = require('exif-parser');
var fs = require('fs');
require('../../secrets');
cloudinary.config({
    cloud_name: process.env.CLOUDINARY_NAME,
    api_key: process.env.CLOUDINARY_KEY,
    api_secret: process.env.CLOUDINARY_SECRET
});
// router.use(
//   cors({
//     origin: 'http://localhost:3000/image'
//   })
// )
router.use(formData.parse());
router.get('/', function (req, res, next) { return __awaiter(_this, void 0, void 0, function () {
    var images, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, Image.findAll()];
            case 1:
                images = _a.sent();
                res.json(images);
                return [3 /*break*/, 3];
            case 2:
                error_1 = _a.sent();
                next(error_1);
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
router.post('/:offset', function (req, res, next) {
    var values = Object.values(req.files);
    var promises = values.map(function (image) {
        var buffer = fs.readFileSync(image.path);
        var parser = EXIFParser.create(buffer);
        var result = parser.parse();
        var orientation = result.tags.Orientation;
        var time = result.tags.DateTimeOriginal;
        //specify given timezone
        var withTimeZone = time.toString() + req.params.offset.toString();
        time = new Date(withTimeZone);
        return cloudinary.uploader.upload(image.path).then(function (cloudRes) {
            return Image.create({
                imageUrl: cloudRes.url,
                time: time.toUTCString(),
                orientation: orientation
            }).then(function (err) { return console.log(err); });
        });
    });
    Promise.all(promises)
        .then(function (results) {
        console.log(results);
        res.json(results);
    })
        .catch(function (err) { return res.status(400).json(err); });
});
router.post('/upload', function (req, res, next) { return __awaiter(_this, void 0, void 0, function () {
    var _a, string, id, seq, gpxEntry;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, string = _a.string, id = _a.id, seq = _a.seq;
                return [4 /*yield*/, Gpx.create({
                        gpxString: string,
                        uniqueId: id,
                        sequence: seq
                    })];
            case 1:
                gpxEntry = _b.sent();
                res.send(gpxEntry);
                return [2 /*return*/];
        }
    });
}); });
module.exports = router;
//# sourceMappingURL=images.js.map